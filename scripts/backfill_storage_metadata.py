...
def backfill(*, apply_changes: bool, upload_local_to_s3: bool) -> tuple[int, int]:
    ...
    try:
        ...
        for doc in rows:
            ...
            changed = False
            uploaded_local_path: Path | None = None
            uploaded_storage_uri: str | None = None
            ...
            if should_migrate:
                ...
                if local_path.is_file():
                    if apply_changes:
                        ...
                        uploaded_storage_uri = stored.storage_uri
                    changed = True
            ...
            if changed:
                ...
                if apply_changes:
                    try:
                        ...
                        db.rollback()
                        if uploaded_storage_uri:
                            try:
                                backend.delete_by_uri(uploaded_storage_uri)
                            except Exception:
                                pass
                        raise
                    if uploaded_local_path:
                        uploaded_local_path.unlink(missing_ok=True)
                else:
                    db.rollback()
                    db.expire_all()
    finally:
