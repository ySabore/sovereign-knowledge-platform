...
class RBACRoleEnforcementTests(unittest.TestCase):
    ...
    def _seed(self) -> None:
        ...
        try:
            ...
            db.add(other_org)
            db.flush()
            other_ws = Workspace(organization_id=other_org.id, name="Other Workspace", description="Foreign org")
            db.add(other_ws)
            db.flush()
            self.cross_org_id = other_org.id
            self.cross_org_workspace_id = other_ws.id
            ...
            db.flush()
            self.cross_org_doc_id = self.cross_org_doc.id
    ...
    def test_workspace_admin_cannot_sync_permissions_for_cross_org_document(self) -> None:
        headers = self._login("ws-admin-rbac@example.com")
                ...
                "items": [
                    {
                        ...
                        "can_read": True,
                        "source": "google-drive",
                        "external_id": "same-org-doc",
                    },
                    {
                        "document_id": str(self.cross_org_doc_id),
                        "organization_id": str(self.cross_org_id),
                        "can_read": True,
                        "source": "google-drive",
        ...
        db = self.SessionLocal()
        try:
            cross_org_permission = (
                db.query(DocumentPermission)
                .filter(DocumentPermission.document_id == self.cross_org_doc_id)
                .one_or_none()
            )
            same_org_permission = (
                db.query(DocumentPermission)
                .filter(DocumentPermission.document_id == self.editor_doc.id)
                .one_or_none()
            )
            self.assertIsNone(cross_org_permission)
            self.assertIsNone(same_org_permission)
        finally:
            db.close()
